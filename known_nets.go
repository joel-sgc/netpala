...
func get_known_networks(conn *dbus.Conn) []known_network {
	...
	for _, d := range devs {
		...
		if t, _ := obj.GetProperty("org.freedesktop.NetworkManager.Device.DeviceType"); t.Value().(uint32) != 2 { continue }
	...
	for _, c := range conns {
		...
		if cobj.Call("org.freedesktop.NetworkManager.Settings.Connection.GetSettings", 0).Store(&s) != nil { continue }
		...
		ss := ssidStr(ssidV)
		hidden := false
		if hv, ok := wcfg["hidden"]; ok { hidden = hv.Value().(bool) }
		auto := false
		if cSec, ok := s["connection"]; ok {
			if av, ok := cSec["autoconnect"]; ok { auto = av.Value().(bool) }
		}
		sec := "Open"
		if wsec, ok := s["802-11-wireless-security"]; ok {
			if km, ok := wsec["key-mgmt"]; ok {
				...
				switch {
				case strings.Contains(kmStr, "sae"): sec = "WPA3-SAE"
				case strings.Contains(kmStr, "wpa-psk"): sec = "WPA/WPA2-PSK"
				case strings.Contains(kmStr, "wpa-eap"): sec = "WPA-EAP"
				case strings.Contains(kmStr, "none"): sec = "WEP/Unknown"
				default: sec = "Encrypted"
				}
			}
		}
		apInfo := aps[ss]
	...
	sort.SliceStable(known, func(i, j int) bool {
		if known[i].connected != known[j].connected { return known[i].connected }
