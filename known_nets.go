...
func get_known_networks() []known_network {
	conn, _ := dbus.SystemBus()
	nm := conn.Object(nmDest, "/org/freedesktop/NetworkManager")

	// helper to decode SSID
	...
	// build a map of SSID -> best AP info
	...
	for _, c := range conns {
		...
		if err := cobj.Call("org.freedesktop.NetworkManager.Settings.Connection.GetSettings", 0).Store(&s); err != nil {
			continue
		}
		...
		known = append(known, known_network{
			ssid:        ss,
			security:    sec,
			connected:   apInfo.connected,
			hidden:      hidden,
			auto_connect: auto,
			signal:      apInfo.signal,
			bssid:       apInfo.bssid,
		})
	}
